<template>
    <div id="add_village">
        <form  class="AddDistrict-form" v-on:submit.prevent="addVillage">
            <h3>Add Village</h3>
            <div class="form-group">
                <select  name="thana_id" id="thana_id" class="box" v-model="villageData.thana_id">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            addVillage: async function(){
                try {

                    let formData = new FormData();

                    formData.append('thana_id', this.villageData.thana_id);
                    formData.append('name_en', this.villageData.name_en);
                    formData.append('name_bn', this.villageData.name_bn);
                    formData.append('code_en', this.villageData.code_en);
                    formData.append('code_bn', this.villageData.code_bn);

                    await this.$store.dispatch('village/add_village', formData).then(() => {
                        this.$swal.fire({
                            toast: true,
                            position: 'top-end',
                            icon: 'success',
